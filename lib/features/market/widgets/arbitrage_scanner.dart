...
/// Cross-market arbitrage scanner — looks for price discrepancies
...
class _ArbitrageScannerState extends State<ArbitrageScanner> {
  ...
  List<_ArbOpportunity> _findArbitrage() {
    ...
    // (i.e., YES price + NO price < 0.95 → buy both for guaranteed profit)
    ...
    for (final m in _markets) {
      ...
      if (spread > 0.02 && yes > 0.05 && yes < 0.95) {
        ...
        final vol = (m['volumeNum'] as num?)?.toDouble() ?? (m['volume24hr'] as num?)?.toDouble() ?? 0;
        final profit = spread * (vol > 0 ? vol * 0.01 : 100); // estimated profit per $100
        ...
        children: [
          ...
          Expanded(
            child: _loading
                ...
                : opps.isEmpty
                    ? Center(child: Text('No arbitrage opportunities', style: TextStyle(color: const Color(0xFF5E6A85), fontSize: 12, fontFamily: PulsColors.fontMono)))
                    : ListView.builder(
...
class _ArbOpportunity {
  ...
  _ArbOpportunity(this.slug, this.yesPrice, this.noPrice, this.spread, this.estProfit);
}
      ...
      child: Row(
        children: [
          Container(
            ...
            decoration: BoxDecoration(color: const Color(0xFF2DD4BF).withValues(alpha: 0.12), borderRadius: BorderRadius.circular(3)),
            child: Text('+$spreadPct%', style: const TextStyle(color: Color(0xFF2DD4BF), fontSize: 10, fontWeight: FontWeight.w800, fontFamily: PulsColors.fontMono)),
          ),
          const SizedBox(width: 8),
          Expanded(child: Text(qShort, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0xFFEAF0FF), fontSize: 10, fontFamily: PulsColors.fontMono))),
          const SizedBox(width: 6),
          Text('${(opp.yesPrice * 100).round()}¢', style: const TextStyle(color: Color(0xFF2DD4BF), fontSize: 10, fontWeight: FontWeight.w700, fontFamily: PulsColors.fontMono)),
          const Text('/', style: TextStyle(color: Color(0xFF5E6A85), fontSize: 10, fontFamily: PulsColors.fontMono)),
          Text('${(opp.noPrice * 100).round()}¢', style: const TextStyle(color: Color(0xFFEC4899), fontSize: 10, fontWeight: FontWeight.w700, fontFamily: PulsColors.fontMono)),
        ],
      ),
    );
  }
